            ...
            @if (userProfile(); as profile) {
              <div class="relative ml-4 flex items-center">
                <button (click)="toggleDropdown()" class="flex items-center gap-2 text-sm rounded-full focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary">
                  <span class="hidden sm:inline font-semibold text-gray-700 text-sm">{{ profile.displayName }}</span>
                  <img class="w-8 h-8 rounded-full" [src]="profile.photoURL || 'assets/images/default-avatar.png'" alt="Foto do Perfil">
                </button>
